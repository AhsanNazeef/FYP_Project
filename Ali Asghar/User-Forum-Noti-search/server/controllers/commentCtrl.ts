...
const commentCtrl = {
  createComment: async (req: IReqAuth, res: Response) => {
    ...
    try {
      ...
      const blogI = await Blogs.findById(blog_id);
      const userNo = await Users.findById(blog_user_id)
      const userFro = await Users.findById(req.user._id)
      if (userNo !== userFro) { userNo.notification.push(`${userFro.username} commented on your ${blogI.title}`) }
  ...
  replyComment: async (req: IReqAuth, res: Response) => {
    ...
    try {
      ...
      const comment1 = await Comments.findById(comment_root);
      const blog1 = await Blogs.findById(blog_id)
      const userNo = await Users.findById(reply_user._id)
      const userFro = await Users.findById(req.user._id)
      if (userNo !== userFro) { userNo.notification.push(`${userFro.username} replied to your ${comment1.content} on blog ${blog1.title}`) }
